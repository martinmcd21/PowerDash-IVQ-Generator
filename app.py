...
if st.button("Generate Interview Pack", type="primary"):
    # Lazy imports so the UI isn't blocked by failures at import time
    try:
        from utils.generation_iqt import generate_interview_pack
    except Exception as e:
        st.error(f"Couldn't load generator module. Check the file exists at utils/generation_iqt.py and requirements are installed.\n\n**Import error:** {e}")
        st.stop()

    try:
        from utils.export_iqt import pack_to_docx, pack_to_pdf
    except Exception as e:
        st.warning(f"Export module failed to load. You can still generate the pack.\n\n**Import error:** {e}")
    ...
    with st.spinner("Drafting your pack…"):
        try:
            ...
            pack = generate_interview_pack(inputs=inputs, model=model, temperature=temperature)
        except Exception as e:
    ...
    if pack_to_docx:
        try:
            docx_bytes = pack_to_docx(pack, tenant_name=tenant_name, logo_url=logo_url)
    ...
    if pack_to_pdf:
        try:
            pdf_bytes = pack_to_pdf(pack, tenant_name=tenant_name, logo_url=logo_url)
...
# ---------- Footer ----------
if 'show_powerdash' in locals() and show_powerdash:
    st.markdown(
        f"""
